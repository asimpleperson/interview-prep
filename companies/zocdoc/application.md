# Zocdoc — Application

## Positions

| Role | Account | Status | Next Step | Notes |
|---|---|---|---|---|
| — *(role TBD)* | — | `Recruiter Screen` | Recruiter call · **2026-04-28 13:30** | Rescheduled from 4/27 |

---

## Active Position — Role TBD

| Field | Value |
|---|---|
| Role | — |
| Team / Org | — |
| Recruiter | — |
| Recruiter Email | — |
| Job Req ID | — |
| Applied | — |
| Status | `Recruiter Screen` |

## Timeline

| Date | Event | Notes |
|---|---|---|
| 2026-04-27 | Recruiter call rescheduled | Moved to 4/28 |
| 2026-04-28 13:30 | Recruiter call | Scheduled |

## Interview Rounds

| Round | Date | Time | Format | Interviewer | Result | Notes |
|---|---|---|---|---|---|---|
| Recruiter Call | 2026-04-28 | 13:30 | Phone | — | Pending | Rescheduled from 4/27 |

---

## Prep Checklist for This Role

### Coding
- [ ] All Must-Have problems
- [ ] Arrays, hash tables, strings
- [ ] Trees and graphs

### System Design
Healthcare marketplace — expect scheduling, data reliability, or integration problems.
- [ ] Appointment scheduling and calendar management
- [ ] Healthcare marketplace: provider availability, patient matching
- [ ] EHR / EMR integrations (HL7, FHIR)
- [ ] Insurance eligibility and benefits verification pipelines
- [ ] HIPAA-compliant data handling and PHI storage

### Behavioral
- [ ] Impact on a mission-driven product
- [ ] Navigating constraints (compliance, legacy integrations)
- [ ] Cross-functional collaboration with clinical or ops teams

### Project Deep Dive
- [ ] Highlight any marketplace, integration, or reliability-focused work

### Company Research
- [ ] See [CLAUDE.md](CLAUDE.md) for Zocdoc-specific notes

---

## Notes / Observations

<!-- Add recruiter conversations, interview feedback, impressions here -->
