# Apple — Application

## Position

| Field | Value |
|---|---|
| Role | Staff Software Engineer |
| Team / Org | — |
| Recruiter | — |
| Recruiter Email | — |
| Job Req ID | — |
| Applied | — |
| Status | `Applied` |

## Timeline

| Date | Event | Notes |
|---|---|---|
| 2026-05-06 | Referred by friend | Application submitted via referral |

## Interview Rounds

| Round | Date | Format | Interviewer | Result | Notes |
|---|---|---|---|---|---|
| — | — | — | — | — | — |

---

## Prep Checklist for This Role

### Coding
- [ ] All Must-Have problems
- [ ] Data Structure Design
- [ ] Trees and graphs

### System Design
Apple interviews are team-specific; prep broadly.
- [ ] iCloud sync / conflict resolution
- [ ] Maps and location services
- [ ] App Store / CDN and large binary delivery
- [ ] Siri / on-device ML serving
- [ ] Privacy-preserving data collection

### Behavioral
Apple values craft, attention to detail, and deep ownership.
- [ ] "Tell me about a project you're most proud of"
- [ ] Handling design disagreements
- [ ] Delivering quality under deadline

### Project Deep Dive
- [ ] Primary project narrative polished

### Company Research
- [ ] See [CLAUDE.md](CLAUDE.md) for Apple-specific notes

---

## Notes / Observations

<!-- Add recruiter conversations, interview feedback, impressions here -->
